...
// Sample private key for testing signature generation
...
$config = [
    ...
    'merchant_app_id' => '1543320856473600',
    'merchant_code' => '155526',
    'notify_url' => 'https://silly-deer-fetch.loca.lt',
    'return_url' => 'http://localhost:8000/payment/success',
    'private_key' => $privateKey,
];

// Logger callable to inspect outputs
$logger = function(string $message, string $level = 'info', array $context = []) {
    echo "[$level] $message " . (empty($context) ? "" : json_encode($context)) . "\n";
};

echo "Initializing TelebirrClient...\n";
$client = new TelebirrClient($config, $logger);

echo "Testing signature generation (calling a protected sign method via reflection or testing standard format)...\n";
$reflector = new ReflectionClass(TelebirrClient::class);
...
try {
    $signature = $method->invoke($client, $testParams);
    echo "Signature generated successfully: " . substr($signature, 0, 40) . "...\n";
    echo "SUCCESS: TelebirrClient class works correctly!\n";
} catch (Exception $e) {
